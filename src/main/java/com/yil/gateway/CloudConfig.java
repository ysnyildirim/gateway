...
public class CloudConfig {
    ...
    public RouteLocator gwRoutes(RouteLocatorBuilder builder) {
        return builder
                ...
                .route("companyModule", r -> r.path("/api/company/v1/**").filters(f -> f.filter(filter)).uri("http://localhost:8084/"))
                .route("organizationModule", r -> r.path("/api/org/v1/**").filters(f -> f.filter(filter)).uri("http://localhost:8085/"))
                .route("contactModule", r -> r.path("/api/contact/v1/**").filters(f -> f.filter(filter)).uri("http://localhost:8086/"))
